...
| [`df2insert.py`](df2insert.py) | Generate Teradata INSERT statements from a pandas DataFrame to populate a volatile table. Companion to `df2table.py` — use together to create and load a volatile table in one workflow. Supports batched `INSERT ... SELECT ... UNION ALL` (default) or individual `INSERT ... VALUES` per row. |
...
> Part of [In One We Trust](https://www.inonewetrust.com) — Faith · Code · Depth. The `excel_ingestor`, `sttm_function`, `pbkdf2`, `df2table`, and `df2insert` scripts are featured on [/code](https://www.inonewetrust.com/code).
